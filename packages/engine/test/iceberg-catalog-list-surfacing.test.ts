...
// SURFACE, not be masked as an empty table list. Swallowing a network/auth
// failure to `[]` makes `listIcebergTables` report "no tables" and
// `dropIcebergTables` quietly drop nothing — both indistinguishable from a
...
const { listIcebergTables, dropIcebergTables } = await import('../src/iceberg/catalog')
...
describe('dropIcebergTables: list failure surfacing', () => {
  it('surfaces a list failure when discovering the default target set', async () => {
    restCatalogListTables.mockRejectedValueOnce(new Error(LIST_FAILURE))
    // No explicit `tables` arg → it must list the namespace; that list failing
    // must NOT degrade to "drop nothing".
