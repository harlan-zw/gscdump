...
import type { AnalysisParams } from 'gscdump/driver'
import process from 'node:process'
import { cancel, isCancel, select } from '@clack/prompts'
import { defineCommand } from 'citty'
import { isCloudDriver } from 'gscdump/driver'
import { hasLocalData, LocalStoreEmptyError, LocalStoreUnsupportedError, runLocalAnalysis } from '../analysis-local'
...
async function resolveSiteUrl(driver: Awaited<ReturnType<typeof getDriver>>, siteUrl?: string): Promise<string> {
...
function makeToolCommand(tool: AnalysisTool): CommandDef<any> {
  ...
  return defineCommand({
    ...
    async run({ args }) {
      ...
      // Cloud mode: use analysisPost for full server-side processing
      if (isCloudDriver(driver)) {
        const data = await driver.analysisPost(siteUrl, params).catch((e: Error) => {
        ...
        renderResults(results, total, format)
        return
      }

      // Local mode: prefer the Parquet store when data is present and the
      // analysis is supported locally. Fall back to live GSC API otherwise.
      const config = await loadConfig()
      if (!args.live) {
        ...
        if (localAvailable) {
          const localResult = await runLocalAnalysis(harness, siteUrl, params).catch((e: Error) => {
            if (e instanceof LocalStoreUnsupportedError) {
              logger.warn(`${e.message}; falling back to live GSC API.`)
              return null
            }
      ...
      // Live mode: use driver.analysis() which queries Google API directly
      const result = await driver.analysis(siteUrl, params).catch((e: Error) => {
        logger.error(`Analysis failed: ${e.message}`)
        process.exit(1)
      })
...
function computeRowSeriesSparkline(results: Record<string, unknown>[]): { spark: string, label: string } | null {
  ...
  if (!metricKey)
    return null
  // Only emit if every row has the bucket + metric
  for (const r of results) {
...
function renderResults(results: Record<string, unknown>[], total: number, format: string): void {
  ...
  // Precompute time-aligned sparklines for any series columns
  ...
  // If the rows themselves form a time series (one bucket per row), print a
  // summary sparkline so e.g. `seasonality` shows a year-at-a-glance trend.
