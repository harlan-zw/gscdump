...
export const runReportInput = z.object({
  ...
  period: z.string().optional().describe('Window: 7d|28d|30d|90d|180d|365d|mtd|qtd|ytd|last-quarter|custom (default per report). Windows end on the newest final GSC date.'),
  comparison: z.string().optional().describe('Comparison: none|prev-period|yoy (default per report). yoy compares with the same weekdays 52 weeks earlier.'),
  start: z.string().optional().describe('Custom window start (YYYY-MM-DD). Implies period=custom.'),
  end: z.string().optional().describe('Custom window end (YYYY-MM-DD). Implies period=custom.'),
  prevStart: z.string().optional().describe('Override comparison-window start. Pass with prevEnd.'),
  prevEnd: z.string().optional().describe('Override comparison-window end. Pass with prevStart.'),
