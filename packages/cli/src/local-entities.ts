...
export async function syncInspections(deps: {
  ...
  const state = await loadInspectionState(deps.dataSource, deps.ctx, deps.now())
  let latest = state.latest
  const candidates = deps.candidates.filter(url => urlInProperty(deps.siteUrl, url))
  const plan = planInspections({ candidates, latest, recent: state.recent, now: deps.now(), limit: deps.limit })
  ...
  while (queue.length > 0 && !stopped) {
    ...
    latest = latestByUrl([...latest.values(), ...records])
