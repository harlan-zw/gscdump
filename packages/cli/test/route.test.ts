...
const covered: NeedCoverage = { kind: 'window', table: 'pages', searchType: 'web', window: WINDOW, stored: true, gaps: [] }
...
describe('decideRoute', () => {
  it.each<[string, Partial<RouteRequest>, Partial<RouteState>, Route]>([
    ...
    ['asks to sync the missing days of partial coverage', {}, { coverage: [partial] }, { kind: 'prompt', reason: { kind: 'partial', done: 2, total: 3, missing: partial.gaps }, nextCommand: sync }],
    ['asks to log in first when partial coverage has no auth', {}, { auth: 'none', coverage: [partial] }, { kind: 'prompt', reason: { kind: 'partial', done: 2, total: 3, missing: partial.gaps }, nextCommand: 'gscdump auth login' }],
    ['never mixes sources when one table is synced and another is not', {}, { coverage: [covered, { ...empty, table: 'queries' }] }, { kind: 'prompt', reason: { kind: 'partial', done: 0, total: 3, missing: [WINDOW] }, nextCommand: 'gscdump sync --site example.com --start 2026-08-01 --end 2026-08-03 --tables queries' }],
    ...
    ['treats a stale sync as not running', {}, { coverage: [partial], syncRun: stale }, { kind: 'prompt', reason: { kind: 'partial', done: 2, total: 3, missing: partial.gaps }, nextCommand: sync }],
    ...
    ['names the non-web search type in the sync command', {}, { coverage: [{ ...partial, searchType: 'image' }] }, { kind: 'prompt', reason: { kind: 'partial', done: 2, total: 3, missing: partial.gaps }, nextCommand: `${sync} --types image` }],
