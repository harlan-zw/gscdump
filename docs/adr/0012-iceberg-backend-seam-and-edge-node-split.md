...
Amended 2026-07-20 for v1: dataset-agnostic catalog connections, schema types,
and raw Iceberg primitives are no longer exported by `./iceberg`; consumers
import them from `@gscdump/lakehouse` (or its explicit `unsafe-raw` subpath).
`./iceberg` now owns only GSC dataset schemas, GSC catalog wrappers, and the
GSC append sink. This completes the major-bump boundary planned by ADR-0021.
...
Iceberg was removed from the root barrel entirely. Import GSC-specific dataset
types/functions from `@gscdump/engine/iceberg`; import generic catalog and
dataset APIs from `@gscdump/lakehouse`. The duplicate `IcebergCatalogConfig`
in the overwrite writer (which omits `catalogToken` because its PyIceberg
backends authenticate differently) was renamed `OverwriteWriterCatalogConfig`;
the generic catalog client keeps `IcebergCatalogConfig`.
