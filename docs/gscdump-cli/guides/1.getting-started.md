...
description: Install the gscdump CLI, authenticate with Google or Bing, and query or export your search data.
---
...
Your agent can use the Skill to install the CLI and help you connect your Search Console account.
For MCP configuration, see [AI integration](./3.ai-integration.md#mcp-client-configuration).

To set up the CLI yourself, follow the steps below.
...
Login opens gscdump.com for Google authorization. You do not need your own Google Cloud project.
Data queries call Google directly; gscdump.com handles login and token refresh.
For your own OAuth client, follow [authentication setup](./2.authentication.md#local-google-credentials).

If you already have an access token, set `GSC_ACCESS_TOKEN` and skip `init`.
For refresh tokens and service accounts, see [CLI authentication](./2.authentication.md#local-google-credentials).
...
See [Bing authentication and exports](../api/5.bing.md) for OAuth, datasets, date limits, and formats.
...
Without dates, `query` reads the last 28 days that end on the newest synced day, or three days ago with `--live`.
...
See [export row limits](https://gscdump.com/learn-google-search-console/limits/export-row-limits) for the limits of each export method.
...
Every analytics row has `site` and `search_type` columns. The Store keeps every search type, so filter or group by `search_type` before you add rows together.
Each dump writes `manifest.json` with every file size and how much of each dataset the Store holds so far. Later syncs continue collecting data within Google’s API limits.
...
- [Keep historical data](./4.historical-database.md)
- [Run SEO Analyzers and Reports](./5.seo-analysis.md)
- [Connect an AI assistant](./3.ai-integration.md)
- [Inspect URLs and manage sitemaps](./6.url-indexing.md)
