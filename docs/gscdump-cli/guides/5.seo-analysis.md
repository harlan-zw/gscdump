---
title: SEO analysis
description: Use gscdump Analyzers and Reports to examine search traffic from live data or a local Store.
---
...
Some Analyzers can read live rows. SQL-only Analyzers need stored data.
...
Reports return a `ReportResult` with Sections of findings and next steps.
...
See the [Report list](../api/7.reports.md) for default windows and required inputs.
...
Analyzer windows default to 28 days ending on the newest synced day.
If either window has missing dates, the command stops and prints a sync command to fill them.
Pass `--live` to read Google instead.
